...
const A = { alpha: 1, scale: 1, dx: 0, dy: 0, rot: 0, streak: 0, hot: 0, blur: 0 };
let mL1 = null, mL2 = null, mPts = -1, mAcc = null, mCol = null;
let mFaces = null, mScale = -1, mSeed = -1, mLk = null;

function lockup(faces, state, scale, seed) {
  ...
  if (mLk && mL1 === l1 && mL2 === l2 && mPts === pts && mAcc === acc && mCol === col
    && mFaces === faces && mScale === scale && mSeed === seed) return mLk;
  mLk = lockupFor(faces, state, { scale, seed });
  mL1 = l1; mL2 = l2; mPts = pts; mAcc = acc; mCol = col;
  mFaces = faces; mScale = scale; mSeed = seed;
  return mLk;
}
 ...
 */
export function drawLockup(c2d, faces, state, ax, ay, scale, seed, ageOverride) {
  const lk = lockup(faces, state, scale, seed);
  if (!lk) return null;
  ...
  if (A.streak > 0.02) {
    for (let i = 5; i >= 1; i--) {
      ...
      c2d.drawImage(lk.cv, -lk.ox + f * 170 * A.streak, -lk.oy - f * 12 * A.streak);
    }
  }
